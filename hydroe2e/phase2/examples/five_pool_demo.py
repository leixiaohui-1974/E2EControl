...
from hydroe2e.phase2.controllers.multi_objective_mpc import MultiObjectiveMPC
from hydroe2e.phase2.topology.network_topology import create_simple_cascade
import time
...
def run_five_pool_simulation(total_hours: int = 50):
    ...
    logger.info(f"   渠池数量: {num_pools}")
    logger.info(f"   闸门数量: {len(system.gates)}")
    logger.info(f"   MPC时域: {controller.horizon}")
    logger.info(f"   ADMM参数: rho={admm_params.rho}, alpha={admm_params.alpha}")
    ...
    logger.info(f"   需求范围: [{np.min(demands):.2f}, {np.max(demands):.2f}] m³/s")
    ...
    for t in range(total_hours):
        ...
        if (t + 1) % 10 == 0:
            ...
            logger.info(f"   进度: {t+1}/{total_hours}h | "
                  f"平均迭代: {avg_iter:.1f} | "
    ...
    for i in range(num_pools):
        ...
        std = np.std(levels)
        logger.info(f"      池{i}: RMSE={rmse:.4f}m, 最大偏差={max_dev:.4f}m, 标准差={std:.4f}m")
    ...
    for i in range(num_pools - 1):
        ...
        logger.info(f"      池{i}→池{i+1}: 平均误差={balance_error:.4f} m³/s, "
    ...
    logger.info(f"      平均求解时间: {avg_solve_time:.2f} ms")
    logger.info(f"      最大求解时间: {max_solve_time:.2f} ms")
    logger.info(f"      收敛率: {convergence_rate:.1f}%")
    ...
    logger.info(f"      供水保证率: {guarantee_rate:.1f}%")
    logger.info(f"      平均偏差: {np.mean(delivery_error):.4f} m³/s")
    logger.info(f"      最大偏差: {np.max(delivery_error):.4f} m³/s")
...
def visualize_five_pool_results(history, num_pools):
    ...
    logger.info("   图表已保存: five_pool_results.png")
    plt.close()
