...
from hydroe2e.phase5.distributed_sil import (
    DistributedSILFramework,
    ScenarioGenerator,
)
from hydroe2e.phase5.distributed_sil.core.sil_framework import SILConfig, SimulationMode
from hydroe2e.phase5.distributed_sil.core.scenario_generator import ScenarioType
...
def demo_steady_state():
    ...
    logger.info(f"持续时间: {scenario.duration/3600:.1f} 小时")

    # 运行仿真
    def progress_callback(progress, result):
        if result.get("kpi"):
            score = result["kpi"].overall_score
            logger.info(f"进度: {progress*100:.1f}%, 评分: {score:.3f}")
    ...
    logger.info(f"  最终评分: {result.final_kpi.overall_score:.3f}")
    ...
    logger.info(f"\n评估统计:")
    logger.info(f"  通过率: {report['evaluation_report']['summary']['pass_rate']*100:.1f}%")
...
def demo_step_response():
    ...
    logger.info(f"场景: 在1小时后流量从300增加到330 m³/s")
    ...
    logger.info(f"\n控制器统计:")
    logger.info(f"  总指令数: {ctrl_stats['total_commands']}")
    logger.info(f"  活跃控制器: {ctrl_stats['active_controllers']}")
...
def demo_flood_scenario():
    ...
    logger.info(f"场景: 洪水过程,峰值流量450 m³/s")
    ...
    logger.info(f"  通过: {result.passed}")
    logger.info(f"  最终评分: {result.final_kpi.overall_score:.3f}")
    logger.info(f"  约束违背数: {result.final_kpi.constraint_violation_count}")
...
def demo_ensemble_simulation():
    ...
    logger.info(f"\n集合仿真结果:")
    logger.info(f"  通过数: {pass_count}/5")
    logger.info(f"  平均评分: {np.mean(scores):.3f}")
...
def demo_manual_step_simulation():
    ...
    for step in range(10):
        ...
        if step % 3 == 0:
            ...
            logger.info(f"  步骤 {step}: {state_summary}")

    # 获取最终报告
    report = framework.generate_report()
    logger.info(f"\n仿真统计:")
    logger.info(f"  总步数: {report['simulation_summary']['total_steps']}")
    logger.info(f"  仿真时间: {report['simulation_summary']['total_time']/3600:.2f} 小时")


def main():
    """主函数"""
    logger.info("="*60)
    logger.info("南水北调中线 分布式SIL框架 演示")
    logger.info("="*60)
