...
def get_config_endpoint() -> Tuple[Response, int]:
    try:
        ...
        return jsonify({'success': False, 'error': 'Invalid configuration'}), 500
    except (OSError, IOError) as exc:
        logger.error("Failed to read config file: %s", exc, exc_info=True)
...
def interpret_instruction_endpoint() -> Tuple[Response, int]:
    ...
    try:
        ...
        logger.error("Semantic interpretation failed: %s", exc, exc_info=True)
        return jsonify({'success': False, 'error': 'Interpretation failed'}), 422
    except ValidationError as exc:
        logger.error("Validation error during interpretation: %s", exc, exc_info=True)
        return jsonify({'success': False, 'error': 'Invalid instruction parameters'}), 400
...
def run_simulation_endpoint() -> Tuple[Response, int]:
    ...
    try:
        ...
        logger.error("Invalid simulation parameters: %s", exc, exc_info=True)
        return jsonify({'success': False, 'error': 'Invalid simulation parameters'}), 400
    except (ConfigurationError, PhysicsError, RuntimeError) as exc:
...
def get_simulation_history_endpoint(sim_id: str) -> Tuple[Response, int]:
    try:
        ...
        logger.error("Data error retrieving history for %s: %s", sim_id, exc, exc_info=True)
...
def _run_simulation_worker(sim_id: str, instruction: str) -> None:
    ...
    except (PhysicsError, ConfigurationError) as e:
        logger.error("Simulation %s failed (domain error): %s", sim_id, e, exc_info=True)
        with sim_lock:
            simulations[sim_id]['status'] = 'failed'
            simulations[sim_id]['error'] = str(e)
    except (ValueError, RuntimeError) as e:
        logger.error("Simulation %s failed (runtime error): %s", sim_id, e, exc_info=True)
        with sim_lock:
            simulations[sim_id]['status'] = 'failed'
            simulations[sim_id]['error'] = str(e)
...
def run_app():
    """CLI入口：启动 HydroE2E REST API 服务。"""
    import os
    host = os.environ.get('E2E_API_HOST', '0.0.0.0')
    port = int(os.environ.get('E2E_API_PORT', '5000'))
    debug = os.environ.get('E2E_API_DEBUG', '').lower() in ('1', 'true')
